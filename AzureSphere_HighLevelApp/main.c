...
{
    nrfResetGpioFd =
        GPIO_OpenAsOutput(AVNET_MT3620_SK_GPIO17, GPIO_OutputMode_OpenDrain, GPIO_Value_High);
    if (nrfResetGpioFd == -1) {
    ...
    nrfUartFd = UART_Open(AVNET_MT3620_SK_ISU0_UART, &uartConfig);
    if (nrfUartFd == -1) {
    ...
    nrfDfuModeGpioFd =
        GPIO_OpenAsOutput(AVNET_MT3620_SK_GPIO2, GPIO_OutputMode_OpenDrain, GPIO_Value_High);
    ...
    triggerUpdateButtonGpioFd = GPIO_OpenAsInput(AVNET_MT3620_SK_USER_BUTTON_A);
